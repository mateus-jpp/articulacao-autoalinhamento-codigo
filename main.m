...
norm_cor = zeros(length(step_batch_v),length(ordem_v));
T = zeros(length(step_batch_v),length(ordem_v));
...
%             acel{1}(k,:) = (rot_ruido*acel{1}(k,:)')';    
    ...
    cor_v(b,o) = cor(1,2);
    norm_cor(b,o) = ((rmse_j(b,o)/0.1226 +rmse_o(b,o)/0.9202)/cor_v(b,o))/4;
    v_erro_j(b+o-1) = rmse_j(b,o);
    v_erro_o(b+o-1) = rmse_o(b,o);
...
med_j = median(v_erro_j)
med_o = median(v_erro_o)
...
if length(step_batch_v) > 1            
    ...
    xlabel('Ordem')
    ylabel('Tamanho Batch')
    zlabel('Erro do vetor deslocamento [m/s²]')
    figure
    surf(ordem_v,step_batch_v,rmse_j)
    grid minor
    xlabel('Ordem')
    ylabel('Tamanho Batch')
    zlabel('Erro do versor direção [graus/s]')
    figure
%     surf(ordem_v,step_batch_v,norm_cor)
%     grid minor
%     xlabel('Ordem')
%     ylabel('Tamanho Batch')
%     zlabel('Soma do erro normalizada')
    surf(ordem_v,step_batch_v,T)
    grid minor
    xlabel('Ordem')
    ylabel('Tamanho Batch')
    zlabel('Tempo de processamento (s)')
else
...
for i = 1:seg-1
    ...
    plot(tempo, rad2deg(alpha_giro_v(i,step_batch+2:end-2))+12,'color',[0.4940 0.1840 0.5560],'LineWidth',2) % Giroscópio
    ...
    grid minor
%     title(['Estimativa ângulo articular - batch = ', num2str(step_batch_v) , ' | ' , 'ordem = ' , num2str(ordem_v)])
    title('Estimativa ângulo articular')
    legend('Acel.','Giro.','Filtro','Encoder')
    xlim([0,31])
    ylim([-10,60])
...
%     xline((9*50-(step_batch+2))*dt,'-r',{'Rotação','Dados'},'HandleVisibility','off')
end
